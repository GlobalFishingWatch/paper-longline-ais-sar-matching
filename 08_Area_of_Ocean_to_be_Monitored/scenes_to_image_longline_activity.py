...
# %%
for rate in [0.05, 0.1, 0.2, 0.25, 0.5, 0.75, 1]:
    sarm.get_images(rate, df)
...
# %%
sarm.map_fraction(df, 0.5)

# %%
sarm.map_fraction(df, 0.1)

# %%
fishing = pyseas.maps.rasters.df2raster(
    df,
    "lon_index",
    "lat_index",
    "fishing_hours",
...
with psm.context(pyseas.styles.light):
    ...
    for row in range(r):
        for col in range(c):
            ...
            pyseas.maps.core._last_projection = cartopy.crs.EqualEarth(
...
with plt.rc_context(psm.styles.light):
    ...
    psm.add_land(facecolor="lightgrey", edgecolor="none")
    ax.set_title(
        f" {index:,} Scenes to Image {int(rate*100)}% of Longline Activity in 2020",
        pad=10,
        fontsize=28,
    )
    psm.add_figure_background()
...
ax2.xaxis.set_major_formatter(mpl.ticker.FuncFormatter(lambda y, p: format(int(y), ",")))
# start, end = ax2.get_xlim()
# ax2.xaxis.set_ticks(np.arange(0, end, 5000))
ax2.plot([3820, 3820], [0, 0.5], "--", c="dimgray")
ax2.text(4000, 0.2, "3,820 scenes\n50% of fishing", size=20)
ax2.tick_params(labelsize=20)
ax2.grid(False)

plt.text(
    0.25, 0.86, "a", fontsize=22, fontweight="bold", transform=plt.gcf().transFigure
)
plt.text(
    0.21, 0.42, "b", fontsize=22, fontweight="bold", transform=plt.gcf().transFigure
)
